...
void print_html(server_info serv[]) {
    ...
    for (int i = 0; i < N_SERVERS; ++i) {
        if (serv[i].exist) {
            cout << "<th scope=\"col\">" << serv[i].hostname << ":" << serv[i].port << "</th>" << endl;
        }
    }
    cout << "</tr>" << endl;
    cout << "</thead>" << endl;
    ...
    for (int i = 0; i < N_SERVERS; ++i) {
        if (serv[i].exist) {
            cout << "<td><pre id=\"" << serv[i].element_id << "\" class=\"mb-0\"></pre></td>" << endl;
        }
    }
    cout << "</tr>" << endl;
    cout << "</tbody>" << endl;
...
void parse_query(server_info serv[]) {
    ...
    for (int i = 0; i < (int)paras.size(); ++i) {
        ...
        if (paras[i][0] != 's') {
            ...
            if (paras[i][0] == 'h') {
                serv[n].hostname = paras[i].substr(index + 1);
            } else if (paras[i][0] == 'p') {
                serv[n].port = paras[i].substr(index + 1);
            } else if (paras[i][0] == 'f') {
                serv[n].filename = "./test_case/" + paras[i].substr(index + 1);
            }
        } else {
            if (paras[i][1] == 'h') {
                socks_host = paras[i].substr(index + 1);
            } else if (paras[i][1] == 'p') {
                socks_port = paras[i].substr(index + 1);
            }
        }
    }

    for (int i = 0; i < N_SERVERS; ++i) {
        if (serv[i].hostname != "" && serv[i].port != "" && serv[i].filename != "") {
            serv[i].element_id = "s" + to_string(i + 1);
            serv[i].fin.open(serv[i].filename, ifstream::in);
            serv[i].exist = true;
        }
    }
}
...
private:
    ...
    void do_write_request(server_info &server) {
        char packet[max_length] = {0};
        packet[0] = 4;
        // connect
        packet[1] = 1;
        // DSTport
        packet[2] = (atoi(server.port.c_str()) >> 8) & 0xFF;
        packet[3] = atoi(server.port.c_str()) & 0xFF;
        // DSTIP
        packet[4] = 0;
        packet[5] = 0;
        packet[6] = 0;
        ...
        // NULL
        packet[8] = 0;
        memcpy(packet + 9, server.hostname.c_str(), server.hostname.size());
        packet[9 + server.hostname.size()] = 0;
        boost::asio::async_write(socket_, boost::asio::buffer(packet, 10 + server.hostname.size()),
            [&, this](boost::system::error_code ec, size_t write_length) {
                if (!ec) {
                    do_read_reply(server);
                } else {
                    cerr << "Write => " << ec << endl;
                }
            });
    ...
    void do_read_reply(server_info &server) {
        socket_.async_read_some(
